...
Collection::Collection(const Collection& c1, const Collection& c2, 
	...
	for_each(c1.member_list.cbegin(), c1.member_list.cend(),
		[&](Record* record) {member_list.insert(record);});
	for_each(c2.member_list.cbegin(), c2.member_list.cend(),
		[&](Record* record) {member_list.insert(record);});
}
...
void Collection::remove_then_add_member(Record* old_record, 
	Record* new_record) {
...
void Collection::save(std::ostream& os) const {
	...
	for_each(member_list.cbegin(), member_list.cend(), 
		[&](const Record* record) {os << record->get_title() << endl;});
}
