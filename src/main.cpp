...
// Find commands
void fr_command(const Lib_ti_t& lib_ti, const Lib_id_t& , const Cat_t&, 
	const int&);
void fs_command(const Lib_ti_t& lib_ti, const Lib_id_t&, const Cat_t&, 
	const int&);

// Print commands
void pr_command(const Lib_ti_t&, const Lib_id_t& lib_id, const Cat_t&, 
	const int&);
void pc_command(const Lib_ti_t&, const Lib_id_t&, const Cat_t& cat, const int&);
void pL_command(const Lib_ti_t& lib_ti, const Lib_id_t&, const Cat_t&, 
	const int&);
void pC_command(const Lib_ti_t&, const Lib_id_t&, const Cat_t& cat, 
	const int&);
void pa_command(const Lib_ti_t& lib_ti, const Lib_id_t&, const Cat_t& cat, 
	const int&);

// List command
void lr_command(const Lib_ti_t& lib_ti, const Lib_id_t&, const Cat_t&, 
	const int&);

// Collection stats & combine commands
void cs_command(const Lib_ti_t& lib_ti, const Lib_id_t&, const Cat_t& cat,
	const int&);
void cc_command(const Lib_ti_t&, const Lib_id_t&, Cat_t& cat, const int&);

// Add commands
void ar_command(Lib_ti_t& lib_ti, Lib_id_t& lib_id, const Cat_t&, 
	int& record_id);
void ac_command(const Lib_ti_t&, const Lib_id_t&, Cat_t& cat, const int&);
void am_command(const Lib_ti_t&, const Lib_id_t& lib_id, Cat_t& cat, 
	const int&);

// Modify command
void mr_command(const Lib_ti_t&, const Lib_id_t& lib_id, const Cat_t&, 
	const int&);
void mt_command(Lib_ti_t& lib_ti, Lib_id_t& lib_id, 
	Cat_t& cat, const int&);

// Delete commands
void dr_command(Lib_ti_t& lib_ti, Lib_id_t& lib_id, const Cat_t& cat, 
	const int&);
void dc_command(const Lib_ti_t&, const Lib_id_t&, Cat_t& cat, const int&);
void dm_command(const Lib_ti_t&, const Lib_id_t& lib_id, Cat_t& cat,
	const int&);

// Function wrappers for cL and cC commands
void cL_command_wrapper(Lib_ti_t& lib_ti, Lib_id_t& lib_id,
	const Cat_t& cat, int& record_id);
void cC_command_wrapper(const Lib_ti_t& lib_ti, const Lib_id_t& lib_id,
	Cat_t& cat, const int& record_id);

// Clear commands
void cL_command(Lib_ti_t& lib_ti, Lib_id_t& lib_id, const Cat_t& cat, 
	int& record_id);
void cC_command(const Lib_ti_t&, const Lib_id_t&, Cat_t& cat, const int&);
void cA_command(Lib_ti_t& lib_ti, Lib_id_t& lib_id, Cat_t& cat, int &record_id);

// Save & restore commands
void sA_command(const Lib_ti_t& lib_ti, const Lib_id_t&, const Cat_t& cat, 
	const int&);
void rA_command(Lib_ti_t& lib_ti, Lib_id_t& lib_id, Cat_t& cat, 
	int& record_id);

// Quit command
void qq_command(Lib_ti_t& lib_ti, Lib_id_t& lib_id, Cat_t& cat, 
	int& record_id);

// Helper functions used for main
void skip_rest_of_line(const char* error_msg);
void print_and_clear_data(const char* error_msg, Lib_ti_t& lib_ti,
	Lib_id_t& lib_id, Cat_t& cat, int& record_id);
...
Cat_citer find_and_check_if_already_present(const Cat_t& cat, 
	const string& name);
...
int main() {
	...
	const map<string, function<void (Lib_ti_t&, Lib_id_t&, Cat_t&, 
		int& record_id)>> 
		command_map =
	{ {"fr", fr_command}, {"fs", fs_command}, {"pr", pr_command}, 
	{"pc", pc_command}, {"pL", pL_command}, {"pC", pC_command}, 
	{"pa", pa_command}, {"lr", lr_command}, {"cs", cs_command}, 
	{"cc", cc_command}, {"ar", ar_command}, {"ac", ac_command}, 
	{"am", am_command}, {"mr", mr_command}, {"mt", mt_command}, 
	{"dr", dr_command}, {"dc", dc_command}, {"dm", dm_command}, 
	{"cL", cL_command_wrapper}, {"cC", cC_command_wrapper}, 
	{"cA", cA_command}, {"sA", sA_command}, {"rA", rA_command}, 
	{"qq", qq_command} };
	...
	while (true) {
		...
		// Clear data and exit for other exceptions
			...
			print_and_clear_data("Memory allocation failure!", 
				lib_ti, lib_id, cat, record_id);
			return 0;
		}
		catch (...) {
			print_and_clear_data("Unknown exception caught!", 
				lib_ti, lib_id, cat, record_id);
			return 0;
		}
	}
...
class Find_string {
	public:
		Find_string() : at_least_one(false) {}
...
void fs_command(const Lib_ti_t& lib_ti, const Lib_id_t&, const Cat_t&, 
	...
	transform(str_to_find.cbegin(), str_to_find.cend(),
		str_to_find.begin(), ::tolower);
...
void lr_command(const Lib_ti_t& lib_ti, const Lib_id_t&, const Cat_t&, 
	...
	for_each(lib_ti.cbegin(), lib_ti.cend(),
		[&](Record* record) {temp.insert(record);});
...
class Collection_stats {
	public:
		Collection_stats() : first_time(true), total_member(0),
			total_at_least_one(0), total_more_than_one(0) {}
...
void cs_command(const Lib_ti_t& lib_ti, const Lib_id_t&, const Cat_t& cat,
	...
	cout << func.get_total_at_least_one() << " out of " << lib_ti.size() <<
	...
	cout << "Collections contain a total of " << func.get_total_member() << 
		" Records" << endl;
}
...
void cc_command(const Lib_ti_t&, const Lib_id_t&, Cat_t& cat, const int&) {
	...
	cout << "Collections " << col_first.get_name() << " and " <<
		col_second.get_name() << " combined into new collection " << name << endl;
...
void am_command(const Lib_ti_t&, const Lib_id_t& lib_id, Cat_t& cat, 
	...
	cout << "Member " << record_ptr->get_ID() << " " <<
		record_ptr->get_title() << " added" << endl;
}
...
void mr_command(const Lib_ti_t&, const Lib_id_t& lib_id, const Cat_t&, 
	...
	cout << "Rating for record " << record_ptr->get_ID() <<
		" changed to " << record_ptr->get_rating() << endl;
}
	...
	// new title and then insert it into the library.
		...
		try {
			new_record = new Record((*record_iter_found)->get_ID(),
				(*record_iter_found)->get_medium(), title,
		...
		cout << "Title for record " << (*record_iter_found)->get_ID() <<
			" changed to " << title << endl;
...
void dr_command(Lib_ti_t& lib_ti, Lib_id_t& lib_id, const Cat_t& cat, 
	...
	if (any_of(cat.cbegin(), cat.cend(), 
		[&](const Collection& col) {return col.is_member_present(*record_iter);}))
		throw Title_error("Cannot delete a record that is a member of a collection!");

	cout << "Record " << (*record_iter)->get_ID() << " " <<
		(*record_iter)->get_title() << " deleted" << endl;
...
void sA_command(const Lib_ti_t& lib_ti, const Lib_id_t&, const Cat_t& cat, 
	...
	if (myfile.is_open()) {
		...
		for_each(lib_ti.cbegin(), lib_ti.cend(), 
			[&](Record* record) {record->save(myfile);});

		myfile << cat.size() << endl;

		//Save each Collection to the file
		for_each(cat.cbegin(), cat.cend(), 
			[&](Collection collection) {collection.save(myfile);});
	...
	if (myfile.is_open()) {
		...
		try {
			...
			for (int j = 0; j < num_collection; ++j) {
				...
				pair<Cat_citer, bool> iter_bool = 
					cat_binary_search(cat, new_collection.get_name());
			...
			for_each(lib_id_backup.cbegin(), lib_id_backup.cend(), 
				[](Record* record) {delete record;});
			...
			for_each(lib_id.cbegin(), lib_id.cend(),
				[](Record* record) {delete record;});
...
pair<Cat_citer, bool> cat_binary_search(const Cat_t& cat, const string& name) {
	Cat_citer iter_found = lower_bound(cat.cbegin(), cat.cend(), name, 
		[&](const Collection& c1, const string&) 
	{return c1.get_name() < name;});
...
Lib_id_iter find_record_iter(const Lib_id_t& lib_id) {
	...
	Lib_id_iter record_iter = lower_bound(lib_id.cbegin(), lib_id.cend(), id,	
		[&](const Record* r1, const int){return r1->get_ID() < id;});
...
class Remove_unneeded_white {
	public:
		Remove_unneeded_white() : initial_white(true),
			add_white(false), white_at_the_end(false), index(0) {}
